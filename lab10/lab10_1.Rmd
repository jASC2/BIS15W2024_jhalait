...
```{r}
ggplot(data=iris, #specify the data
       mapping=aes(x=Species, y=Petal.Length))+ #map the aesthetics (e.g. x and y axes)
...
In big data sets with lots of overlapping values, over-plotting can be an issue (points begin to clump together). `geom_jitter()` is similar to `geom_point()` but it helps with over plotting by adding some random noise to the data and separating some of the individual points.
```{r}
...
```{r}
homerange %>% 
  ggplot(aes(x=trophic.guild)) + #by default, there is only one variable on this plot (the categorical one)
  geom_bar() #good for counts
```

### Bar Plot: `geom_col()`
Unlike `geom_bar()`, `geom_col()` allows us to specify an x-axis and a y-axis (i.e. plot a continuous value for categories)
```{r}
homerange %>% 
