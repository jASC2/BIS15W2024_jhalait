...
```{r}
spider_list[["Butte"]]
#you can now store this as an individual object with the following command
...
inner_exampleDF #inner join only matches the observations for which all columns are present in both data sets
```
...
right_exampleDF #the right table serves as the absolute/the observations that'll be included, for which the cell will add the necessary columns from the left table
```

`full_join`
```{r}
full_exampleDF <- full_join(table_A, table_B, by="customer_ID") #the by value must match exactly and be unique on each data set.
full_exampleDF
```
...
```{r}
dmy(spiders_with_locs$Date) #if the format isn't what it expected, the function will reassign that cell "NA."
```
