...
life_history %>%
  group_by(order) %>%
  summarize(mean_mass = mean(mass, na.rm = T)) %>%
  ggplot(aes(x=reorder(order,mean_mass), y=mean_mass)) + #reorder is added when establishing the axes; it takes the variable to reorder and the numeric variable by which to reorder it
  geom_col() +
  scale_y_log10() #add this as an additional option 
```
