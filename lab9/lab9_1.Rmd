...
#not tidy because here, the heart rate variable is split across multiple columns
```
...
column names are actually observations rather than variable names
...
relig_income %>%
  pivot_longer(-religion, 
               names_to = "income", 
               values_to = "total")
#now, every column is its own variable (religion, income, and total) and every observation is in a row
```
...
billboard3 <- 
  billboard %>% 
  pivot_longer(-c(artist, track, date.entered), #specific columns to stay fixed as a vector
               names_to = "week",
               values_to = "rank",
               values_drop_na = TRUE
               )
...
```{r}
#In this data set, the day number is not recorded in a single column
```
...
qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3,
    names_to= c("experiment", "replicate"), #you want to separate the names to two new columns, as they correspond to two types of data (experiment and replicate #)
    names_sep="_",
