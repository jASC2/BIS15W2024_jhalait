...
```{r}
names(hbirds) #informs you of the names of the columns in the data frame
```

What are the dimensions of the `hbirds` data frame? The `dim()` and `str()` commands provide this information.   
```{r}
dim(hbirds) #informs you of the data frame's dimensions
#columns = variables
#cells = observations
```

```{r}
str(hbirds) #provides an alternate way to view the data frame
```
...
#format is name=corresponding vector object
```
...
```{r}
?colMeans
mean_temp_C <- mean(hsprings$temp_C)
mean_temp_C
