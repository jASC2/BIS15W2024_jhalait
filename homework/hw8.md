...
```r
sydneybeaches <- read_csv("../lab9/data/sydneybeaches.csv") %>% clean_names()
```
...
Yes, the data appears to be in a tidy/long format, considering each column contains values for a single variable.
...
```r
sydneybeaches_yearly_means_long <- sydneybeaches_long %>%
  group_by(site, year) %>%
...
```r
syndeybeaches_yearly_means_wide <- sydneybeaches_yearly_means_long %>%
...
Using the code from Q8, Little Bay Beach was the most polluted beach in Sydney in 2013.
