...
There seems to be NAs in the data set that were wrongfully labeled as -999, -999.0000, -999.00, etc.
...
It seems as though the makers of this data set appropriately labeled missing values with NA.
...
The amniota data set appears to be more problematic, as the summary function used above suggests there are no missing values in any of the columns/variables; in reality, using the results of the "glimpse" function used in 2, it appears that the makers of the data set used different variants of -999 to represent NA for most columns.
...
In contrast, the amphibio data frame seems better organized, as nearly all of its columns appear to have at least 1 NA value.
...
This information makes some sense biologically--particularly for mammalia as it is known that nearly all mammals do not lay eggs. That being said, the still high percentage of NAs in the Aves and Reptilia classes does appear strange, however, those likely just indicate species lacking recorded data rather than species that do not lay eggs. In this way, 'NA' can mean different things, such as a value missing or a measurement not being applicable to an observation.
...
I think an assignment of NA to a cell in one of these columns indicates that species does not share that particular quality.
