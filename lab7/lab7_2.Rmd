...
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #here, you specify to conduct the operation on all variables containing the word "mm"
```

```{r}
penguins %>%
  summarize(across(contains("mm"), \(x) mean(x, na.rm = TRUE))) #use this to correct the error that appears 
```
