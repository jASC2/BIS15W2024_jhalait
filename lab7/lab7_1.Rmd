...
```{r}
summary(msleep24) #this can be used to double check the operation was carried out effectively, as you can do a sense-check of the min/median/max sleep_total_24
```
...
Histograms of a numeric variable are also a quick way to check the output.
```{r}
...
summary_table <- msleep %>%
  ...
  summarize(mean_sleep_lg = mean(sleep_total), 
            ...
            total = n()) #total tells you the total number of rows in the data frame fed as an argument
```
...
msleep %>%
  ...
  select(order, genus, brainwt)
# This shows that nobody has measured the brain weight of any whale, leading all of them to be removed in the calculation, which results in an "NA" being assigned to that order.
```
