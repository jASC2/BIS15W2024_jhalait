...
## Load the tidyverse
For the remainder of the quarter, we will work within the `tidyverse`. At the start of each lab, the library needs to be loaded as shown below. Never install a package more than once, but you can load libraries as often as you want.  
...
Every variable is in its own column, every observation is in its own row, and every cell contains a single value. This is called 'tidy data'.  
...
```{r}
select(fish, "lakeid", "scalelength") # order in function = data frame, variable(s) of interest (in ""s since they are variables). The variables are listed in the order you WANT their respective columns to appear. This will limit the output to only the two columns of interest (lakeid and scalelength).
```
...
```{r}
select(fish, contains("length")) #outputs only columns that contain the word "length"
```

When columns are sequentially named, `starts_with()` makes selecting columns easier.
```{r}
select(fish, starts_with("radii")) #there is another function that works similarly called "ends_with()"
```
...
```{r}
select(fish, matches("a.+er")) #handy piece of code for the project. This outputs any columns that being with an "a" and end in an "er".
```
...
```{r}
select_if(fish, ~!is.numeric(.))
#~ means to look at each column in the data frame; ! means "that are not"; "." means to look through each cell
```
...
#read_csv will alter the variable names if they are repeated
```
...
is.na(mammals$`max. life`) #returns a vector with False or True at each position
anyNA(mammals) #returns True or False based on the entire data set
```
...
```{r}
summary(mammals)
# shows that there appears to be an issue in the data entry (they likely put an absurd value rather than an NA in cells with no value)
```
...
```{r}
select_if(mammals_new, ~is_character(.))
```
...
```{r}
library(janitor)
clean_names(mammals) # this function from the janitor package cleans up the naming convention
```
