...
```{r}
## This is a code block aka a place for code in a markdown document. To make these easier to read, add an empty line between them.
y <- c(5, 7, 7, 3, 2, 5, 5, 5)
```
...
```{r}
## To remove an object from the environment, either clear the entire environment or use the following command:
q <- 2
rm(q)
```
...
```{r}
## These reports are very clean, uniform, and powerful. They can be produced by clicking 'knit.'
```
...
```{r}
#install.packages("tidyverse") you do not need to install tidyverse more than once
library("tidyverse")
#this loads the tidyverse library onto your computer
```
