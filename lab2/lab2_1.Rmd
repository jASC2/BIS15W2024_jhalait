...
```{r}
(2+8+2)/3 #this calculates the arithmetic mean (there are other types too, as illustrated below)
mean(2+8+2) #will ask R to find the mean of the sum (12)
```
...
```{r}
mean(venom_GT, chiron, veyron) #the answer here is different because mean is using a different mean definition
```

```{r}
#alternatively you can use 'c()' to concatenate the three objects into a vector of numeric values
...
To find out what type of data you are working with, use the `class()` function. This is important because sometimes we will need to change the type of data to perform certain analyses. Some functions require the use of a specific data classes.
```{r}
class(my_numeric)
```
...
```{r}
my_integer <- 
  as.integer(my_numeric) #create a new object specified as an integer by changing the data type
```
...
The mean function is not applicable to this vector because the 'NA' value in it cannot be used in math operation.
Even if you have many, many values in a vector, it's mean cannot be calculated if there is a single 'NA' in it.


4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm=T) #na.rm removes the NA values in the vector, which allows us to calculate the mean of all other values in the vector
#na.rm means to remove the NA value; we want this, so we specify the conditon as T.
```
