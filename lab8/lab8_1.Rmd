...
```{r}
penguins %>% 
  tabyl(island, species) #part of the janitor package but accomplishes the same thing as counts or group_by/summarize(n()).
```
...
penguins %>%
  summarize(across(!c(species, island, sex, year), 
                   mean, na.rm=T)) #across's first argument is the filtering conditions. This is then followed by the function you want to apply, and finally any options for that command.
```
